...
const getBalances = async (syncWallet: zksync.Wallet) => {
	...
	const verifiedBallances = account.verified.balances;
	return verifiedBallances;
};
...
const getTransactionFee = async (syncProvider: zksync.Provider, walletAddress: string, token: string) => {
	...
	const decimals = 18;
	const fee = await syncProvider.getTransactionFee('Transfer', walletAddress, token);
	return parseInt(fee.totalFee.toString()) / 10 ** decimals;
};

const getWithdrawalFee = async (syncProvider: zksync.Provider, walletAddress: string, token: string) => {
	/*
	 * Returns the gas fee associated with transferring tokens to user
	 * */

	const decimals = 18;
	const fee = await syncProvider.getTransactionFee('Withdraw', walletAddress, token);
	return parseInt(fee.totalFee.toString()) / 10 ** decimals;
};
