...
<template>
  <div>
    ...
    <div>
      <spinner v-model="showSpinner" v-bind:status="status"></spinner>
      <h2 class="title text-white">{{ $t("common.CONFIRM_TRANSACTION") }}</h2>
      <div class="white__container">
        <div class="container">
          <p class="subtitle py-2" v-if="store.transactionDetails">{{ chainName }}</p>
            ...
            <div class="details-group">
              ...
              <p class="text">
                {{
                  roundFormatter(
                    (Number(
                      store.transactionDetails.gasPrice ||
                        store.transactionDetails.maxFeePerGas
                    ) *
...
export default class SignTx extends mixins(Global, Authenticated) {
  ...
  get chainName() {
    if (
      this.store.transactionDetails &&
      Number(this.store.transactionDetails.chainId) === 21
    ) {
    ...
    if (
      this.store.transactionDetails &&
      Number(this.store.transactionDetails.chainId) === 1
    ) {
    ...
    if (
      this.store.transactionDetails &&
      Number(this.store.transactionDetails.chainId) === 42
    ) {
